import { ArgsTable, Story } from '@storybook/addon-docs/blocks';

import { parameters } from '../../../../.storybook/preview';
import Canvas from '../../../../.storybook/components/Canvas';
import { Column, Row } from '../../..';

# Description

Description modules provide a means to orderly layout short-form content.

- [Overview](#overview)
- [Subsection title](#subsection-title)
- [Component API](#component-api)
- [Grid placement](#grid-placement)
- [Examples](#examples)
- [References](#references)
- [Troubleshooting](#troubleshooting)

## Overview

The description module can be built using a combination of the [`TitleBar`](#references) component and [paragraph elements](#references).

Everything passed in as a [render prop](#references) child of `Description` will be rendered.

<Canvas>
  <Story
    id="patterns-unstable-layout-modules-description--default"
    parameters={parameters}
  />
</Canvas>

## Subsection title

If necessary, subsection titles can be used below section titles.

<Canvas>
  <Story
    id="patterns-unstable-layout-modules-description--subsection-title"
    parameters={parameters}
  />
</Canvas>

## Component API

<ArgsTable />

Additional props passed into `Description` will be forwarded along to the underlying description module container.

## Grid placement

Description modules span across 8 columns, following the [narrow grid](#references) implementation.

<Row>
  <Column>
    <img alt="#" src="#" />
  </Column>

  <Column>
    <img alt="#" src="#" />
  </Column>
</Row>

```jsx
<Column lg={8}>
  <Description>
    {({ getLayoutProps }) => (
      <>
        <TitleBar title="Section title" />

        <p {...getLayoutProps()}>
          Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer
          finibus tortor eget est ornare, a ultrices risus tincidunt.
          Suspendisse condimentum mauris at ornare tempor. Phasellus a arcu
          ante. Morbi vitae ultrices quam, eget eleifend magna. Morbi quis
          porttitor ex, in elementum tellus. In eget rutrum eros, eu scelerisque
          risus. Phasellus vel pretium lorem, ut laoreet sapien. Cras ac purus
          vitae velit efficitur iaculis. Nunc.
        </p>

        <p {...getLayoutProps()}>
          Lorem ipsum dolor sit amet, consectetur adipiscing elit. Integer
          finibus tortor eget est ornare, a ultrices risus tincidunt.
          Suspendisse condimentum mauris at ornare tempor. Phasellus a arcu
          ante. Morbi vitae ultrices quam, eget eleifend magna. Morbi quis
          porttitor ex, in elementum tellus. In eget rutrum eros, eu scelerisque
          risus. Phasellus vel pretium lorem, ut laoreet sapien. Cras ac purus
          vitae velit efficitur iaculis. Nunc.
        </p>
      </>
    )}
  </Description>
</Column>
```

## Examples

[Detail layout](?path=/story/patterns-unstable-layout-examples-beta--detail)

## References

- [TitleBar](?path=/docs/patterns-unstable-layout-modules-titlebar)
- [The Paragraph element](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/p)
- [Grid](https://react.carbondesignsystem.com/?path=/docs/grid--narrow)

## Troubleshooting

Visit our [GitHub](https://github.com/carbon-design-system/ibm-security) to view and submit issues, or reach out on [Slack](https://app.slack.com/client/T1V6D3RT7/CRV39QKP0) for additional support.
