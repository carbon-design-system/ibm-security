...
The description list module can be built using a combination of the <LinkTo kind="components-typelayout" story="default">`TypeLayout`</LinkTo> and <LinkTo kind="patterns-layout-modules-titlebarmodule" story="default">`TitleBarModule`</LinkTo> components.

Everything passed in as a child of `DescriptionListModule` will be rendered.

<Canvas>
  <Story
    id="patterns-layout-modules-descriptionlistmodule--default"
...
If necessary, both section and subsection titles can precede the type layout. There are two values already supplied for this so you shouldn't supplement your own type style. Learn more by reviewing the <LinkTo kind="patterns-layout-modules-titlebarmodule" story="default" > TitleBarModule. </LinkTo>

<img alt="`DescriptionListModule` variants" src={variants} />

<Source id="patterns-layout-modules-descriptionlistmodule--variant" />

### Grid placement

Description list modules span across 8 columns, following the [narrow grid](https://react.carbondesignsystem.com/?path=/docs/grid--narrow) implementation.
